...
const _SELF_SCOPE: StringName = "Network"
...
class Client extends Network:
	...
	func _init(host: String = "", port: int = 0) -> void:
		super()
		_host = host
		_port = port


	func start(address: String = _host, port: int = _port) -> Error:
...
class Server extends Network:
	...
	func _init(host: String = "", port: int = 0, max_clients: int = 4) -> void:
		super()
		_peers = {}
		_next_id = 0
		_host = host
	...
	func start(address: String = _host, port: int = _port, max_clients: int = _max_clients) -> Error:
		_enet_host = ENetConnection.new()
