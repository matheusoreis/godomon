...
func list_characters() -> void:
	...
	if not Accounts.has(sender_id):
		_network.exec(sender_id, &"Alert", ["You need to be logged in"])
		return

	var account: Account = Accounts.account(sender_id)
...
func select_character(character_id: int) -> void:
	...
	Characters.add(sender_id, character)
...
func move_character(direction: Vector2i) -> void:
	...
	if not Characters.has(sender_id):
		_network.exec(sender_id, &"Alert", ["No character selected"])
		return

	var character: Character = Characters.character(sender_id)
	var map: Map = Maps.map(character.map)

	if map == null:
		_network.exec(sender_id, &"Alert", ["Map not found"])
		return

	if not Characters.move(sender_id, direction, map):
